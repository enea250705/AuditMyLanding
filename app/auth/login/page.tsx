...
export default function LoginPage() {
  ...
  async function handleGoogle() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${process.env.NEXT_PUBLIC_APP_URL}/auth/callback?next=${redirectTo}` },
    })
  }
    ...
    <div className="min-h-screen bg-bg flex flex-col items-center justify-center p-5">
      ...
      <div className="bg-surface border border-border rounded-2xl p-8 w-full max-w-sm">
        ...
        <p className="text-slate-400 text-sm text-center mt-5">
          ...
          <Link href="/auth/signup" className="text-accent hover:text-accent-light transition-colors">Sign up</Link>
        </p>
      </div>
    </div>
  )
